...
const sendVerificationEmail = async (user, code) => {
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    logger.warn("Email credentials missing; skipping verification email send.");
    return;
  }

  const mailOptions = {
    from: process.env.EMAIL_FROM || "Coco <noreply@coco-travel.app>",
    to: user.email,
    subject: "Coco verification code",
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #2d2d2d;">
        <h2 style="color: #8b5e3c;">Your Coco verification code</h2>
        <p>Use the code below to confirm your email address and unlock your travel planner.</p>
        <div style="font-size: 28px; font-weight: 700; letter-spacing: 0.3em; margin: 20px 0; color: #8b5e3c;">${code}</div>
...
const login = async (req, res) => {
  try {
    ...
    const requiresEmailVerification = !user.isEmailVerified && !user.googleId;
...
const googleLogin = async (req, res) => {
  try {
    ...
    const picture = payload.picture || "default-avatar.jpg";
    ...
    await user.addRefreshToken(refreshTokenHash, deviceInfo.device, deviceInfo.ip);
...
const verifyEmail = async (req, res) => {
  try {
    ...
    if (isExpired || user.emailVerificationCode !== String(code).trim()) {
